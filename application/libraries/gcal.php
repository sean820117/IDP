...
class gcal {
	...
	function __construct() {
		global $apiConfig;
		//$this->config->load('gcalconfig');
		$this->CI =& get_instance();

		
		// load API Client
		require_once "calendar/src/apiClient.php";
		require_once "calendar/src/contrib/apiCalendarService.php";
	...
	public function eventInsert($configObj = array()) {
		...
		$postBody->setSummary($configObj['summary']);
		$postBody->setLocation($configObj['location']);
		$start = new EventDateTime();
		$start->setDate($configObj['start']);
		$start->setTimeZone('Asia/Taipei');
		$postBody->setStart($start);
		$end = new EventDateTime();
		$end->setDate($configObj['end']);
